...
def ensemble_learning(train_data, dev_data):
    ...
    with open('../stacking_post_models/knn_model.pkl', 'rb') as f:
        knn_model = pickle.load(f)
    with open('../stacking_post_models/logistic_regression_model.pkl', 'rb') as f:
        logistic_regression_model = pickle.load(f)
    with open('../stacking_post_models/naive_bayes_model.pkl', 'rb') as f:
        naive_bayes_model = pickle.load(f)
    with open('../stacking_post_models/random_forest_model.pkl', 'rb') as f:
    ...
    # Plot learning curve
    learning_curve.plot_learning_curve_cv(ensemble_model, train_data)


    # Fit the ensemble model
    ensemble_model.fit(X_train_scaled, y_train)

    with open('../post_decision/ensemble_complex_model.pkl', 'wb') as f:
        pickle.dump(ensemble_model, f)

# Make predictions on the dev set
    y_dev_postd = ensemble_model.predict(X_dev_scaled)

    # Print classification report for development set
    print("Classification Report on Development Set:")
    print(classification_report(y_dev, y_dev_postd))

    # Compute confusion matrix for development set
    cm = confusion_matrix(y_dev, y_dev_postd)
